...
class Latent_Embedding(nn.Module):
    def __init__(self, flag, device='cuda'):  # flag = true train, false test
        ...
        for mode in ['train', 'test']:
            self.weight_embedding["split_" + mode] = nn.Embedding(n_samples, n_modes).requires_grad_()  # n_modes 100

        init_main_modes = torch.randn(n_modes, embed_dim)  # (100,240)
    ...
    def forward(self, idx):
        ...
        mode_weights = self.weight_embedding["split_" + self.flag](torch.tensor(idx, device='cuda'))
